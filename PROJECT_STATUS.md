...
- 用户可直接双击 `总控台.app` 使用，或将其拖入程序坞固定；首次从互联网下载到其他 Mac 时仍需完成 Gatekeeper 授权。
...
2026-08-14 07:50 +0800 | Codex
- 修改：原生启动器实现已提交为 `e864998`，同步将 CONSOLE-TASK-006 与 CONSOLE-DES-005 标记为已验收。
- 验证：干净 Git 边界 `make release-check` 通过 16 项检查、170 个 Python 测试和 7 个 JavaScript 测试；仅保留既有素材人工复核提醒。
- 下一步：推送本地 `main` 到用户仓库并核对远程提交。
